...
class Adam(Optimizer, Serializable):
    def __init__(self, lr=0.001, beta1=0.9, beta2=0.999, eps=1e-08, weight_decay=0, amsgrad=False):
        self.lr = lr
        self.beta1 = beta1
        self.beta2 = beta2
        self.eps = eps
    ...
    def get_optimizer(self, param_groups):
        return torch.optim.Adam(param_groups, self.lr, (self.beta1, self.beta2), self.eps, self.weight_decay, self.amsgrad)


class Adafactor(Optimizer, Serializable):
    def __init__(self, lr=None, eps1=1e-30, eps2=1e-3, clip_threshold=1.0, decay_rate=-0.8, beta1=None,
                 weight_decay=0.0, scale_parameter=True, relative_step=True, warmup_init=False):
        self.lr = lr
        self.eps1 = eps1
        self.eps2 = eps2
    ...
    def get_optimizer(self, param_groups):
        return AdafactorImpl(param_groups, self.lr, (self.eps1, self.eps2), self.clip_threshold,
                             self.decay_rate, self.beta1, self.weight_decay, self.scale_parameter,
                             self.relative_step, self.warmup_init)


class AdafactorImpl(torch.optim.Optimizer):
